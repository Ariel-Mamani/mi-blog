<x-app-layout>
    <div class="max-w-5xl mx-auto mt-8">
        ...
        @forelse ($posts as $post)
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-6 transition duration-200 hover:shadow-lg
                {{ !$post->habilitated ? 'opacity-50 grayscale' : '' }}">
                @if($post->imagen)
                    <div class="-mx-6 -mt-6 mb-6">
                        <img src="{{ asset($post->imagen) }}" alt="{{ $post->titulo }}" class="w-full h-64 object-cover rounded-t-lg mb-4">
                    </div>
                @else
        ...
        @empty
    <p class="text-center text-russian_violet ">No has publicado ningún post todavía.</p>
@endforelse
